...
import { getCdnResolvRegionsNodesListApi } from '@vca/apis/cdn'
...
const handleConfirm = async () => {
  formRef.value?.validate().then(async () => {
    ...
    if (validate.flat(Infinity).filter(Boolean).length) {
      return false
    }
    ...
    if (data.rule) {
      return await handleEditorUser(data)
    } else {
...
const handleEditorUser = async (formState: any) => {
  if (formState?.rule_id) {
    return await edit(formState)
  } else {
    return await add(formState)
  }
}
//重置
const resetFunc = () => {
