...
export const validateHandler = <T = string | number | string[] | number[]>(
  ...
  if (!Array.isArray(value)) {
    return handler((String(value || '') as string | number).toString())
  }
...
export const regexpMap = {
  // 内网 IP
  privateIP:
    /^(127\.\d{1,3}\.\d{1,3}\.\d{1,3})|(localhost)|(10\.\d{1,3}\.\d{1,3}\.\d{1,3})|(172\.((1[6-9])|(2\d)|(3[01]))\.\d{1,3}\.\d{1,3})|(192\.168\.\d{1,3}\.\d{1,3})$/,
  // ipv4地址
...
// 验证IPv4地址 内网地址
export const validatePrivateIpv4: FormValidator = (value, callback) => {
  callback(
    validateHandler(value, data =>
      regExpValidateHandler(data, regexpMap.privateIP, '内网IP地址格式错误')
    )
  )
}

// 验证IPv4 外网地址地址
export const validatePublicIpv4: FormValidator = (value, callback) => {
  callback(
    validateHandler(value, () => {
      // 是 IPV4 地址，且不是内网地址
      if (
        regexpMap.ipv4.test(String(value)) &&
        !regexpMap.privateIP.test(value.toString())
      ) {
        return undefined
      }
      return '公网IP地址格式错误'
