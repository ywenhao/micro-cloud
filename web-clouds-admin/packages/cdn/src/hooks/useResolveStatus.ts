import {
  getCdnnodeNodesResolveStatusApi,
  getCdnSiteResolveStatusApi
} from '@vca/apis/cdn'
...
export const useResolveStatus = (module: 'site' | 'node') => {
  if (!['site', 'node'].includes(module)) {
    throw new Error('module must be "site" or "node"')
  }
  // 存储对应的所有的ID
  ...
  const asyncGetResolveStatus = (key: string) => {
    ...
    const params = module === 'node' ? { node_id: key } : { site_id: key }
    run(params)
      .then(res => {
