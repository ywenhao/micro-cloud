<template>
  <a-modal
    modal-class="vc-dialog form-dialog"
    title-align="start"
    v-model:visible="modelValue"
    :title="title"
    :width="520"
    @before-open="handleModalBeforeOpen"
    :on-before-ok="handleConfirm"
    @cancel="handleCancel"
    ok-text="确定"
    cancelText="取消"
    :esc-to-close="false"
    :mask-closable="false"
    :ok-button-props="{ loading }"
    :cancel-button-props="{ loading }"
  >
    <a-form ref="formRef" :model="formState" :rules="rules" layout="vertical">
      <a-form-item
        asterisk-position="end"
        validate-trigger="change"
        field="username"
        label="账号"
      >
        <a-input
          v-model="formState.username"
          placeholder="请输入账号"
          :max-length="18"
          show-word-limit
        ></a-input>
      </a-form-item>
      <a-form-item
        asterisk-position="end"
        validate-trigger="change"
        field="email"
        label="邮箱"
      >
        <a-input v-model="formState.email" placeholder="请输入邮箱"></a-input>
      </a-form-item>

      <a-form-item
        v-if="!isEdit"
        asterisk-position="end"
        validate-trigger="input"
        field="password"
        label="密码"
      >
        <a-input-password
          :key="String(modelValue)"
          v-model="formState.password"
          placeholder="请输入密码"
          :max-length="64"
        ></a-input-password>
      </a-form-item>
      <a-form-item
        asterisk-position="end"
        validate-trigger="change"
        field="group_id"
        label="分组"
      >
        <a-select
          :options="groups"
          placeholder="请选择用户分组"
          v-model="formState.group_id"
        />
      </a-form-item>
      <a-form-item
        asterisk-position="end"
        validate-trigger="change"
        class="mb-0!"
        hide-label
        field="api_status"
      >
        <div class="flex items-center gap-x-2">
          <label>对外接口</label>
          <a-switch
            :unchecked-value="2"
            :checked-value="1"
            :disabled="isSelfUser(formState?.id)"
            v-model="formState.api_status"
          ></a-switch>
        </div>
      </a-form-item>
      <a-form-item
        asterisk-position="end"
        validate-trigger="change"
        class="mb-0!"
        hide-label
        field="status"
      >
        <div class="flex items-center gap-x-2">
          <label>状态</label>
          <a-switch
            :unchecked-value="2"
            :checked-value="1"
            :disabled="isSelfUser(formState?.id)"
            v-model="formState.status"
          ></a-switch>
        </div>
      </a-form-item>
    </a-form>
  </a-modal>
</template>

<script setup lang="ts">
import {
  type FieldRule,
  type FormInstance,
  Message,
  type SelectOptionData
} from '@arco-design/web-vue'
import {
  addUserApi,
  type AdminModel,
  updateUserApi,
  type UserModel
} from '@vca/apis/pedestal'
import { pick } from 'lodash-es'
import { passwordValidate } from 'vc-material'
import { computed } from 'vue'

const formRef = ref<FormInstance>()

const appStore = useAppStore()

const modelValue = defineModel()
const groups = defineModel<SelectOptionData[]>('groups', { default: undefined })
const record = defineModel<AdminModel>('record', { default: undefined })
const loading = defineModel<boolean>('loading', { default: false })
const emit = defineEmits<{
  (event: 'reload'): void
}>()
const isEdit = computed(() => !!record.value?.id)
const isSelfUser = (id: string) => {
  return isEdit.value && appStore.userInfo?.id === id
}
const title = computed(() => (record?.value ? '编辑用户' : '添加用户'))

const formState = ref({
  id: '',
  // 用户名 4-18位，包括英文字母、数字、下划线、@符号
  username: '',
  email: '',
  // 分组
  group_id: undefined,
  // 备注信息，非必填，200字内；
  remark: '',
  password: DEFAULT_PASSWORD,
  status: 1,
  api_status: 2
})

const rules = computed<Record<string, FieldRule[]>>(() => {
  const res: Record<string, FieldRule[]> = {
    username: [
      {
        validator(value, callback) {
          const reg = /^[A-Za-z0-9_]{4,18}$/
          if (!value && !formState.value?.email) {
            callback('请输入账号或邮箱')
          } else if (!reg.test(value)) {
            callback('请输入4-18个字符，可包含英文字母、数字、下划线')
          } else {
            callback()
          }
        }
      }
    ],
    email: [
      {
        type: 'email',
        message: '请正确输入邮箱',
        validator(value, callback) {
          if (!value && !formState.value?.username) {
            callback('请输入账号或邮箱')
          } else if (!value) {
            callback('请输入邮箱')
          } else {
            callback()
          }
        }
      }
    ],
    group_id: [{ required: true, message: '请选择角色' }],
    password: [{ required: true, validator: passwordValidate }],
    remark: []
  }
  return res
})

const handleCancel = () => {
  modelValue.value = false
}
const handleModalBeforeOpen = () => {
  formState.value.id = ''
  formState.value = {
    ...formState.value,
    ...(record.value || { id: undefined })
  }
  if (!formState.value?.id) {
    formState.value.group_id = groups.value?.find(
      item => item?.is_default === 1
    )?.id
  }
}
watchEffect(() => {
  if (!modelValue.value) {
    formRef.value?.resetFields()
  }
})

/** 编辑提交事件 */
const handleEditorSubmit = async () => {
  const pickFields = formState.value?.id
    ? ['id', 'status', 'remark', 'group_id', 'api_status', 'username', 'email']
    : [
        'id',
        'username',
        'status',
        'group_id',
        'password',
        'email',
        'api_status'
      ]
  const params: UserModel = formState.value?.id
    ? pick(formState.value, pickFields)
    : pick(formState.value, pickFields)

  const action = params?.id ? updateUserApi : addUserApi
  try {
    await action(params as any)
    Message.success(params?.id ? '修改成功' : '添加成功')
    emit('reload')
    return true
  } catch (e) {
    return false
  }
}

const handleConfirm = async () => {
  const validate = await formRef.value?.validate()
  if (validate) return false
  return handleEditorSubmit()
}
</script>
