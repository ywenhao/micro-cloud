...
import type { WorkflowSpliceConf } from '@vca/apis/mts'
import { isEmpty } from 'lodash-es'

import type { NodeTypeKeys } from '@/components/flow/types'

import CustomPreset from './video-splice/CustomPreset.vue'
import PresetTemplate from './video-splice/PresetTemplate.vue'
...
watch(
  ...
  val => {
    if (!val) return
    active.value =
      isEmpty(formState.value) || formState.value.splice_preset_id ? 0 : 1
  },
  { immediate: true }
)
