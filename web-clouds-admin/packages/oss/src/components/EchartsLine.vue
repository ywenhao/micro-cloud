  ...
  <div class="flex flex-col">
    <div class="flex items-center justify-between">
      ...
      <a-radio-group
        ...
        @change="handleChange"
      >
        <a-radio value="put_val">外网流出</a-radio>
        <a-radio value="get_val">外网流入</a-radio>
      </a-radio-group>
    </div>
...
const { el: chartRef, onUpdate, instance } = useCharts()
...
const emit = defineEmits<{
  (event: 'upload-status', status: string): void
}>()
...
const legend = defineModel<string[]>('legend', {
  required: true
})

const status = ref<string>('put_val')

const handleChange = (val: string | number | boolean) => {
  emit('upload-status', val as string)
}
/** 列 */
  ...
  tooltip: {
    ...
    formatter(params: any[]) {
      let spans = `<p class="mb-1 mt-0 px-[6px] text-ct-2 text-xs">${formatUnix(
        params[0].name,
        `${props.isShowTime ? 'YYYY-MM-DD' : 'YYYY-MM-DD HH:mm:ss'}`
      )}</p>`
