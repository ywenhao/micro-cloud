    ...
    <div class="w-1/2 flex flex-col gap-y-1">
      ...
      >
        <span class="flex items-center text-xs text-ct-1">
          ...
          ></span>
          <span class="w-140px truncate"
            >{{ xAxisData[index] }}
            <span class="pl-2">{{
              ipListData?.length ? ipListData[index] : ''
            }}</span></span
          >
        </span>
        <span class="pl-4 text-left text-xs text-ct-1 font-medium">{{
          convertByte(item)
        }}</span>
      </div>
    </div>
