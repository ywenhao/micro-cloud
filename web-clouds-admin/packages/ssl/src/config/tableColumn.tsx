...
import { dealDisplay, dealStatusColor } from '@/utils/handleColumn'
            ...
            {{
              ...
              content: () => (
                <div class="flex flex-col items-start max-w-[400px] gap-y-1">
                  {dealDomain(record as ApplyListItem).map((item, index) => {
                    return (
                      <div key={`${item}__${index}`} class="w-full text-ct-1 ">
                        {item || '-'}
                      </div>
                    )
                  })}
