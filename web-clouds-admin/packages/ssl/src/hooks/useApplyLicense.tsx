...
export default function useApplyLicense(
  ...
  const batchApply = async () => {
    ...
    const data: Record<string, any> = {
      ...
      algorithm: formState.value.algorithm,
      domain_list: formState.value.domain!.split('\n').filter(Boolean)
    }
    try {
      loading.value = true
      ...
      content: () => (
        <div class="h-[328px] ">
          ...
          <div class="error-domain-container">
            <Scrollbar style="display:block;height:252px;overflow:auto;">
              {list.map(item => (
  ...
  const submitApply = async () => {
    const map: Record<ApplyCommand, () => void> = {
      ...
      verify: () => {
        if (formState.value.is_verify === 1 || isEdit.value) {
          editApply()
          return
        }
        handleDnsPass()
  ...
  function handleRenew() {
    // if (formState.value.is_verify === 2 && isEdit.value) {
    //   editApply()
    //   return
    // }
    loading.value = true
