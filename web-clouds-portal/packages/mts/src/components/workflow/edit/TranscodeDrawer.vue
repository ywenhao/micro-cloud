...
import type { WorkflowVideoConf } from '@vcp/apis/mts'
import { isEmpty } from 'lodash-es'

import type { NodeTypeKeys } from '@/components/flow/types'

import CustomPreset from './transcode/CustomPreset.vue'
import PresetTemplate from './transcode/PresetTemplate.vue'
...
watch(
  ...
  val => {
    if (!val) return
    active.value =
      isEmpty(formState.value) || formState.value.video_preset_id ? 0 : 1
  },
  { immediate: true }
)
