...
import {
  addMtsPresetSpliceApi,
  getMtsPresetInfoDetailApi,
  type SplicePresetModel,
  updateMtsPresetSpliceApi
} from '@vcp/apis/mts'
import { isEmpty, merge } from 'lodash-es'
import { nameReg } from 'vc-material'

import VideoSpliceFormItem from '@/components/workflow/form/video-splice/VideoSpliceFormItem.vue'
import {
  filterState,
  initBackState,
  initState
} from '@/components/workflow/utils/videoSplice'
...
const formState = ref<SplicePresetModel>(initState())

const state = useSessionStorage(ADD_FORM_STATE_KEY, ref<SplicePresetModel>({}))
...
async function handleSubmit() {
  ...
  loading.value = true
  const apiFn = isEdit.value ? updateMtsPresetSpliceApi : addMtsPresetSpliceApi
...
function back() {
  router.push({ name: 'Presets', params: { type: 'video-splice' } })
}
...
async function getDetail() {
  ...
  try {
    ...
    merge(
      formState.value,
      initBackState(res?.splice_preset! as SplicePresetModel)
    )
  } finally {
    loading.value = false
