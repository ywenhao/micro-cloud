<template>
  <div class="space-setting-table mt-4">
    ...
    >
      ...
      <template #url="{ record }">
        <span
          v-for="(item, index) in record.callback_urls"
          :key="`url__${index}`"
        >
          <span>{{ item }}</span>
...
import { delEventApi, eventListApi, type EventListModel } from '@vcp/apis/oss'
import { cloneDeep } from 'lodash-es'
import { VcModal } from 'vc-material'
import { useRoute } from 'vue-router'

import { eventColumns } from '../columns'
import EventRules from './drawer/EventRules.vue'
...
const dataSource = ref<EventListModel[]>([])
const pageList = ref<EventListModel[]>([])
const pageData = ref({
  pageSize: 5,
  currentPage: 1,
  total: 0
