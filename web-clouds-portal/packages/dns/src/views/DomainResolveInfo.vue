...
import { columns } from '@/columns/resolveInfo'
import EditResolveDrawer from '@/components/resolve/EditResolveDrawer.vue'
import { filterBarScheme } from '@/scheme/resolveInfo'
...
const {
  ...
  data: tableData,
  run: getList
} = useListRequest(recordListApi, queryState)
...
async function getResolveInfo(id: string) {
  ...
  try {
    info.value = await domainInfoApi({ id })
...
async function handleSwitchChange(row: ResolveModel) {
  ...
  await updateRecordApi({ id: row.id!, status })
  Message.success('修改成功')
  getList()
}
