    ...
    >
      <template v-if="currentStep === 1">
        ...
        >
          ...
          <template v-if="!isBatch">
            <a-select
              ...
              @change="v => handleSelectChange(v as number)"
            >
        ...
        <a-form-item asterisk-position="end" field="algorithm" label="加密算法">
          <a-radio-group v-model="formState.algorithm">
            <a-radio
      ...
      <template v-else>
        ...
        <a-steps direction="vertical" class="verify-steps">
          ...
          <a-step status="wait">
            ...
            <template #description>
              <span
                >如果您已在域名控制台配置好DNS解析，请点击“验证”检查信息是否正确</span
              >
              <br />
              <a-button
                type="primary"
...
function handleVerify(row: LicenseInfo) {
  if (!row.is_challenged || row.is_challenged === 2) {
    if (row.is_verify === 1) {
      domainDnsAutoVerify()
    } else {
      dnsLogVerifyFail()
...
:deep(.arco-steps-label-horizontal) {
  .arco-steps-item.arco-steps-item-process .arco-steps-item-title::after {
    background-color: #c9cdd4 !important;
  }
  .arco-steps-item.arco-steps-item-finish .arco-steps-item-title::after {
    background-color: #c9cdd4 !important;
  }
}
</style>
