    ...
    >
      <template #prefix>
        <template v-if="isOwn === 1">
          <a-button type="primary" @click="handleUpload">上传自有证书</a-button>
        </template>
        <template v-else>
          <a-button
            type="primary"
            @click="handleApplyAndVerify('apply', false)"
          >
            申请证书
          </a-button>
    ...
    >
      ...
      <template #pagination-left>
        ...
        <a-dropdown @select="(v)=>handleCommand(v as Command,selectedKeys)">
          <a-button type="outline" :disabled="selectedKeys.length < 1">
          ...
          <template #content>
            <a-doption
              ...
              @click="handleApplyAndVerify('renew', false, selected)"
              >批量续期</a-doption
            >
            <a-doption :disabled="batchDisabled('download')" value="download"
              >批量下载</a-doption
            >
            <a-doption :disabled="selected.length < 1" value="delete"
              >批量删除</a-doption
            >
            <a-doption
              v-if="isOwn === 2"
              :disabled="batchDisabled('revoke')"
              value="revoke"
              >批量吊销证书</a-doption
            >
            <a-doption
...
import { editLicenseApi, licenseListApi } from '@vcp/apis/src/ssl'
...
import UploadLicense from '@/components/business/UploadLicense.vue'
...
const { detailDraw, handleCommand } = useTableColumn(handleUpdate, deleCallback)
...
function deleCallback(isBatch: boolean) {
  if (isBatch) {
  ...
  const map: Record<string, () => boolean> = {
    renew: () => {
      selected.value.find(item => {
        ...
        if (item.is_verify === 2 || disableRenew.includes(item.status ?? 1)) {
          disabled = true
          return disabled
        } else {
          disabled = false
